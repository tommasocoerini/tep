...
# 2. CSS AGGIORNATO (Fix Colori Sidebar + Layout Titolo)
st.markdown(f"""
    <style>
    ...
    /* Stile per i titoli sopra i menu a tendina */
...
# --- SIDEBAR ---
with st.sidebar:
    st.markdown('<p class="sidebar-section-title">Seleziona Sales Representative</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p class="sidebar-section-title">Seleziona Cliente</p>', unsafe_allow_html=True)
...
if not df_display.empty:
    ...
    for _, row in df_view.iterrows():
        ...
        qty_cell = f'<span style="color:#8899BB;">0</span>' if qty_rest == 0 else str(int(qty_rest))
